...
mod tests {
    ...
    #[test]
    fn encode_decode_null_real_and_invalid_binary_marker() {
        let value = json!({
            "none": null,
            "float": 1.5,
            "bad_binary": {"__binary": "not base64"}
        });
        ...
        assert_eq!(decoded["float"], 1.5);
        assert_eq!(decoded["bad_binary"], "");
    }

    #[test]
    fn decode_large_unsigned_integer_as_string() {
        let xml = br#"<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict><key>large</key><integer>9223372036854775808</integer></dict></plist>"#;

        let decoded = decode_plist(xml).expect("decoding failed");

        assert_eq!(decoded["large"], "9223372036854775808");
    }
}
