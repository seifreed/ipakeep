...
impl FileKeychain {
    ...
    /// Create a file keychain with a custom config directory.
    #[cfg(test)]
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn save_account_refuses_auth_file_symlink() {
        let dir = TempDir::new().unwrap();
        let keychain = FileKeychain::with_dir(dir.path().to_path_buf());

        std::os::unix::fs::symlink("/tmp/ipakeep-auth-target", keychain.auth_file()).unwrap();

        let result = keychain.save_account(&test_account()).await;
